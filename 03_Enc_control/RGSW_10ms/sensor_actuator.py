...
TCP_HOST = "127.0.0.1"
TCP_PORT = 9000
TCP_TIMEOUT_S = 0.02   # 로컬호스트이므로 왕복 5ms 내외 목표 / 타임아웃시 u_remote 무시

# ---- 전송 제한 각도(도) ----
ALPHA_DEG_LIMIT = 15.0
...
scopeVoltage.attachSignal(name='u_remote (V)', width=1, color=np.array([0, 0, 255]))
#endregion
...
'''
F = np.array([
    [ -0.1356, -0.0899, 0.0138, -0.0061 ],
    [ 0.0025, -0.2365, 0.0037, 0.0039 ],
    [ -30.3003, -16.0078, 1.7518, -1.2260 ],
    [ 2.0500, -49.0352, 0.7444, -0.2083 ],
], dtype=np.float64)
...
H = np.array([ 5.4291, -29.4978, 1.5083, -2.4745 ], dtype=np.float64)  # shape (4,)
