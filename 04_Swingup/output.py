...
# F = np.array([
#     [ 0.7129, -0.8546, 0.0792, -0.0736 ],
#     [ 0.2512, -0.4106, 0.0588, -0.0529 ],
#     [ 22.8227, -83.0339, 6.9522, -7.3858 ],
#     [ 26.3458, -86.9710, 5.9276, -6.3324 ]
# ], dtype=np.float64)
...
H = np.array([ -23.6890, 72.9728, -5.2922, 6.5933 ], dtype=np.float64)  # shape (4,)
K = H
...
def control_loop():
    ...
    with QubeClass(hardware=hardware, pendulum=pendulum, frequency=frequency) as myQube:
        ...
        while timeStamp < simulationTime and not KILL_THREAD:
            ...
            # y = np.array([theta, alpha], dtype=np.float64) # output feedback에 사용
            ...
            x_meas = np.array([theta, alpha, theta_dot, alpha_dot], dtype=np.float64) # full state sensor

            # full state 피드백
            u = float(K @ x_meas)
            VMAX = 5.0 # high gain obsv에서는 이걸로
            ...
            if count >= countMax:
                ...
                scopePendulum.sample(timeStamp, [alpha, theta])
                # 베이스: theta(측정), theta_hat(추정)
                scopeBase.sample(timeStamp, [alpha_dot, theta_dot])
