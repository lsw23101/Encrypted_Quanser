## observer.py
# Aero2 2-DOF: Observer-Based LQR Tracking Control (R. Fellag et al., 2024)
#
# 제어 구조 (CDSL re-encryption 형태):
#   u(k)        = H * xhat(k) + N_bar * ref
#   xhat(k+1)   = F * xhat(k) + G * y(k)
#
# State:  x = [theta_p, theta_y, dp, dy]'
# Output: y = [theta_p, theta_y]'   (위치만 측정, 속도는 추정)
...
FREQUENCY   = 50          # Hz
Ts          = 1.0 / FREQUENCY
VMAX        = 15.0        # V  (MATLAB clamp ±15)
PITCH_LIMIT = np.deg2rad(40)

# ── Step Reference (MATLAB: pitch=10deg, yaw=-10deg) ───────────────────
REF_PITCH_DEG =  20.0    # deg
REF_YAW_DEG   = -20.0    # deg
...
# ZOH 이산화 (Bd만 필요: 옵저버 feedforward 보정용)
M = np.zeros((6, 6))
M[:4, :4] = A * Ts
M[:4, 4:] = B * Ts
Me = expm(M)
...
# ── Observer-Based Controller Matrices (MATLAB model.m 결과) ───────────
# xhat(k+1) = F*xhat(k) + G*y(k) + P*ref(k),   P = Bd @ N_bar
# u(k)      = H*xhat(k) + N_bar*ref

F = np.array([
    [ 0.3529, -0.0010,  0.0190, -0.0002],
    [-0.0009,  0.3101, -0.0002,  0.0185],
    [-5.3539, -0.0953,  0.9037, -0.0174],
    [-0.0937, -6.3200, -0.0160,  0.8462],
], dtype=np.float64)
...
P = Bd @ N_bar   # (4×2): observer feedforward gain

print("=== Controller ready ===")
print(f"  ref: pitch={REF_PITCH_DEG:.1f}°, yaw={REF_YAW_DEG:.1f}°")

# ── Scope ──────────────────────────────────────────────────────────────
scope = MultiScope(rows=3, cols=2, title='Aero2 Observer-Based LQR (Fellag 2024)', fps=30)

scope.addAxis(row=0, col=0, timeWindow=12, yLabel='Pitch (deg)')
scope.axes[0].attachSignal(name='theta_p (meas)')
scope.axes[0].attachSignal(name='theta_p (est)')
...
scope.axes[1].attachSignal(name='theta_y (est)')
...
def control_loop():
    ...
    startTime = time.time()
    timestamp = 0.0
    xhat      = np.zeros(4, dtype=np.float64)   # 옵저버 상태 초기화

    try:
        while timestamp < RUN_TIME and not KILL_THREAD:
            loop_start = time.perf_counter()

            # ── 1. 센서 읽기 (위치 → 제어에 사용, 속도 → 비교 플롯용) ──
            myAero2.read_analog_encoder_other_channels()
            theta_p =  myAero2.pitchAngle
            theta_y =  myAero2.yawAngle
            dp_meas =  myAero2.pitchRate   # 플롯 비교용 (제어에 미사용)
            dy_meas =  myAero2.yawRate     # 플롯 비교용 (제어에 미사용)
            ...
            # ── 3. 제어 입력: u = H*xhat + N_bar*ref (observer-based) ───
            u_ctrl = H @ xhat + N_bar @ ref_now
            ...
            # ── 6. Scope (측정값 vs 옵저버 추정값 비교) ──────────────────
            scope.axes[0].sample(timestamp, [np.rad2deg(theta_p),   np.rad2deg(xhat[0]), np.rad2deg(ref_now[0])])
            scope.axes[1].sample(timestamp, [np.rad2deg(theta_y),   np.rad2deg(xhat[1]), np.rad2deg(ref_now[1])])
            scope.axes[2].sample(timestamp, [np.rad2deg(dp_meas),   np.rad2deg(xhat[2])])
            scope.axes[3].sample(timestamp, [np.rad2deg(dy_meas),   np.rad2deg(xhat[3])])
            scope.axes[4].sample(timestamp, [V_main])
            scope.axes[5].sample(timestamp, [V_tail])

            # ── 7. 옵저버 State Update ────────────────────────────────────
            xhat = F @ xhat + G @ y + P @ ref_now
