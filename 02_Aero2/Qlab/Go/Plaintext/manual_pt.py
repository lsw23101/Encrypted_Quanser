...
#
# 통신 흐름 (매 스텝):
#   1. recv u        (Controller → Plant)  2 floats  ← controller가 먼저 전송
#   2. apply u       (write_voltage)        클리핑은 여기서만 수행
#   3. read sensor   (hardware tick @ 50Hz)
#   4. send (u, y)   (Plant → Controller)  4 floats  u_echo + y
#   ※ r은 controller.go 내부에서 계산
#   ※ 클리핑은 write_voltage 직전에만 — state 업데이트에는 원본 u 사용
# -----------------------------------------------------------------------
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            while timestamp < RUN_TIME and not KILL_THREAD:
                ...
                #       controller가 먼저 u를 전송하는 구조
                u_vals = recv_floats(sock, 2)
                if u_vals is None:
                    break

                # ── 2. safety check & apply u ─────────────────────────────
                #       클리핑은 여기(write_voltage 직전)에서만 수행
                #       state 업데이트용 u_echo는 클리핑 전 원본 사용
                ...
                # ── 3. read sensors (blocks at 50 Hz hardware tick) ───────
                ...
                # ── 4. send [u_echo, y] to controller (4 floats) ──────────
                #       u_vals(클리핑 전 원본) + y를 함께 전송
                #       controller는 u_echo를 state 업데이트에 사용
                send_floats(sock, list(u_vals) + list(y))
