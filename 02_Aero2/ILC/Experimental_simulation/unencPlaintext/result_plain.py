"""result_plain.py — Plot first vs last plaintext ILC iteration from Plaintext/data/.
...
Saves a vector-format PDF to data/result_plain_k<first>_k<last>.pdf
"""
...
fig.suptitle('Plaintext ILC — Learning Progress', fontsize=12, y=0.99)
fig.subplots_adjust(
...
# ── Save as PDF (vector) ──────────────────────────────────────────────────
out_path = os.path.join(DATA_DIR, f'result_plain_k{k1}_k{kl}.pdf')
