"""
plot_test.py — test.go 결과 시각화 (FOLDER=test, 20 trials)
4-panel: output tracking / tracking error / V_ILC / control input
"""
...
if ref is None:
    raise FileNotFoundError(f"ref.csv not found in {FOLDER}/  (run test.go first)")
...
if not available:
    raise FileNotFoundError(f"No trial_*_y.csv in {FOLDER}/")
...
print(f"Ts={Ts}s  Trial {first_tr} vs {last_tr}  ({n_err} trials)")
...
fig.suptitle(f'Test Run — Trial {first_tr} vs Trial {last_tr}  ({n_err} trials)',
             fontsize=9, y=0.99)
...
C1 = '#77BB88'; C2 = '#005522'; C3 = '#CC8877'; C4 = '#990000'

ax_y.plot(t, ref_flat, 'k--', lw=0.9, label='Reference')
...
ax_y.set_ylabel('$y$'); ax_y.set_title('Output Tracking')
ax_y.legend(ncol=4, loc='upper right'); ax_y.grid(True)

ax_e.axhline(0, color='k', lw=0.5, ls='--')
if y_first is not None:
    e1 = ref_flat - y_first.flatten()
    ax_e.plot(t, e1, color=C1, lw=1.0, label=f'Trial {first_tr}  RMS={np.sqrt(np.mean(e1**2)):.4f}')
if y_last is not None:
    eN = ref_flat - y_last.flatten()
    ax_e.plot(t, eN, color=C2, lw=1.2, label=f'Trial {last_tr}  RMS={np.sqrt(np.mean(eN**2)):.4f}')
ax_e.set_ylabel('$r-y$'); ax_e.set_xlabel('Time (s)'); ax_e.set_title('Tracking Error')
ax_e.legend(ncol=3, loc='upper right'); ax_e.grid(True)
...
ax_v1.set_ylabel('$v_{\\mathrm{ILC},1}$ (V)'); ax_v1.set_xlabel('Time (s)')
ax_v1.set_title('$V_{\\mathrm{ILC}}$ — ch 1'); ax_v1.legend(); ax_v1.grid(True)
ax_v2.set_ylabel('$v_{\\mathrm{ILC},2}$ (V)'); ax_v2.set_xlabel('Time (s)')
ax_v2.set_title('$V_{\\mathrm{ILC}}$ — ch 2'); ax_v2.legend(); ax_v2.grid(True)
...
ax_u1.set_ylabel('$u_1$ (V)'); ax_u1.set_xlabel('Time (s)')
ax_u1.set_title('Control Input — ch 1'); ax_u1.legend(); ax_u1.grid(True)
ax_u2.set_ylabel('$u_2$ (V)'); ax_u2.set_xlabel('Time (s)')
ax_u2.set_title('Control Input — ch 2'); ax_u2.legend(); ax_u2.grid(True)

out = os.path.join(FOLDER, 'results_summary.pdf')
