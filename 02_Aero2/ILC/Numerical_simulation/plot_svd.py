"""
plot_svd.py — SVD ILC (simulation_svd.go) result plots
...
FOLDER = os.path.join(BASE, 'result_svd')
...
# Line styles: black dashed = reference, red dotted = first trial, blue solid = last trial
# Draw order: blue (zorder=1) → red (zorder=2) → reference (zorder=3)
S_REF   = {'color': 'k',       'ls': ':',  'lw': 1.8, 'zorder': 3}
S_FIRST = {'color': '#CC5500', 'ls': '--', 'lw': 1.4, 'zorder': 2}
S_LAST  = {'color': '#1A8C3C', 'ls': '-',  'lw': 1.5, 'zorder': 1}

# ── Output tracking ───────────────────────────────────────────────────────────
if y_last is not None:
    ax_y.plot(t, y_last.flatten(),  label=f'Trial {last_tr}',  **S_LAST)
if y_first is not None:
    ax_y.plot(t, y_first.flatten(), label=f'Trial {first_tr}', **S_FIRST)
ax_y.plot(t, ref_flat, label='Reference', **S_REF)
ax_y.set_ylabel('$y$')
...
if y_last is not None:
    ...
    ax_e.plot(t, eN, label=f'Trial {last_tr}',  **S_LAST)
if y_first is not None:
    e1 = ref_flat - y_first.flatten()
    ax_e.plot(t, e1, label=f'Trial {first_tr}', **S_FIRST)
ax_e.set_ylabel('$r - y$')
...
if v_first is not None and t_v is not None:
    ax_v1.plot(t_v, v_last[0::2],  label=f'Trial {last_tr}',  **S_LAST)
    ax_v1.plot(t_v, v_first[0::2], label=f'Trial {first_tr}', **S_FIRST)
    ax_v2.plot(t_v, v_last[1::2],  label=f'Trial {last_tr}',  **S_LAST)
    ax_v2.plot(t_v, v_first[1::2], label=f'Trial {first_tr}', **S_FIRST)
ax_v1.set_ylabel('$v$')
...
if u_last is not None:
    ax_u1.plot(t, u_last[:, 0],  label=f'Trial {last_tr}',  **S_LAST)
    ax_u2.plot(t, u_last[:, 1],  label=f'Trial {last_tr}',  **S_LAST)
if u_first is not None:
    ax_u1.plot(t, u_first[:, 0], label=f'Trial {first_tr}', **S_FIRST)
    ax_u2.plot(t, u_first[:, 1], label=f'Trial {first_tr}', **S_FIRST)
ax_u1.set_ylabel('$u$')
...
out = os.path.join(FOLDER, 'results_svd.pdf')
