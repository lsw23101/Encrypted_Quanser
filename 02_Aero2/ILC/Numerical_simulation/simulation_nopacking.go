...
// simulation_nopacking.go — Encrypted ILC, unpacked RGSW in ILC update
//   Controller (F,G,H,R,P,Nbar) stays packed (same as simulation_packed.go).
//   ILC update (K_r × E) uses RGSW.Enc / RGSW.Mult (no packing).
//   Purpose: compare ILC update latency vs simulation_packed.go.
//
// Run: go run simulation_nopacking.go
...
func main() {
	...
	// ── SVD: Range(G_N^T) basis ────────────────────────────────────────────
	fmt.Println("[SVD] Computing S via Gram-Schmidt...")
	maxRowNormSq := 0.0
	for i := 0; i < pN; i++ {
		norm := 0.0
		for k := 0; k < mN; k++ {
	...
	fmt.Printf("[SVD] rank(G_N) = nR = %d  (pN=%d, mN=%d)\n", nR, pN, mN)

	// ── Quantisation ──────────────────────────────────────────────────────
	scaleS := 1.0 / 1000.0
	scaleL := 1.0 / 100.0
	scaleR := 1.0 / 1000.0
	fmt.Printf("Scaling: 1/s=%v  1/r=%v  1/L=%v\n", 1/scaleS, 1/scaleR, 1/scaleL)
	...
	fmt.Printf("[ILC] γ=%.6f  (1/λ_max, λ_max=%.4f)  nR=%d  K_r: %d×%d\n",
		gamma, lambdaMax, nR, len(Kr), len(Kr[0]))
	fmt.Printf("[ILC] ρ(I - γ·G_r^T·G_r) = %.6f  (stable: %v)\n", rhoSVD, rhoSVD < 1.0)
	...
	// ILC uses no packing — no tauSVD needed
	fmt.Printf("Packing: tauCtrl=%d  (ILC unpacked, nR=%d)\n", tauCtrl, nR)
	...
	// Galois keys for tauCtrl only (ILC update needs none)
	galEls := make([]uint64, lognCtrl)
	for i := 0; i < lognCtrl; i++ {
		galEls[i] = uint64(tauCtrl/int(math.Pow(2, float64(i))) + 1)
	}

	ilcStateDir := filepath.Join(baseDir, "ilc_state_timing_nopacking")
	...
	// ── Encrypt controller matrices (packed, same as simulation_packed.go) ─
	ctF := RGSW.EncPack(F_, tauCtrl, encryptorRGSW, levelQ, levelP, ringQ, params)
	ctG := RGSW.EncPack(utils.ScalMatMult(1/scaleS, G_), tauCtrl, encryptorRGSW, levelQ, levelP, ringQ, params)
	ctH := RGSW.EncPack(utils.ScalMatMult(1/scaleS, H_), tauCtrl, encryptorRGSW, levelQ, levelP, ringQ, params)
	ctR := RGSW.EncPack(utils.ScalMatMult(1/scaleS, R_), tauCtrl, encryptorRGSW, levelQ, levelP, ringQ, params)
	ctNbar := RGSW.EncPack(utils.ScalMatMult(1/scaleS, Nbar), tauCtrl, encryptorRGSW, levelQ, levelP, ringQ, params)
	ctP := RGSW.EncPack(utils.ScalMatMult(1/scaleS, P_), tauCtrl, encryptorRGSW, levelQ, levelP, ringQ, params)

	fmt.Printf("Steps: %d  (Ts=%.3fs, T=%.1fs)\n", nSteps, Ts, trajTime)

	// ── K_r encryption: NO PACKING (RGSW.Enc) ────────────────────────────
	fmt.Println("[NoPack-ILC] Encrypting K_r with RGSW.Enc...")
	tSetup := time.Now()
	ctKr := RGSW.Enc(utils.ScalMatMult(1/scaleS, Kr), encryptorRGSW, levelQ, levelP, params)
	// ctKr: [][]*rgsw.Ciphertext of shape nR × pN
	fmt.Printf("[NoPack-ILC] ctKr ready in %v  (nR=%d × pN=%d)\n",
		time.Since(tSetup).Round(time.Millisecond), len(ctKr), len(ctKr[0]))

	// ── Init ILC state: nR scalar RLWE CTs ───────────────────────────────
	zeroThetaBar := make([]int64, nR)
	ThetaEncCt := RLWE.Enc(zeroThetaBar, 1/scaleL, *encryptorRLWE, ringQ, params)
	// ThetaEncCt: []*rlwe.Ciphertext of nR scalar elements
	thetaVec := make([]float64, nR)
	vIlcVec := make([]float64, mN)

	// Load plaintext theta from previous run (re-encrypt with current key)
	thetaCSVPath := filepath.Join(ilcStateDir, "theta_nopacking.csv")
	if data, err := loadCSV(thetaCSVPath); err == nil && len(data) >= 1 && len(data[0]) >= nR {
		thetaVec = data[0][:nR]
		thetaBar := make([]int64, nR)
		for i := range thetaBar {
			thetaBar[i] = int64(math.Round(thetaVec[i] / (scaleR * scaleS)))
		}
		ThetaEncCt = RLWE.Enc(thetaBar, 1/scaleL, *encryptorRLWE, ringQ, params)
		vIlcVec = utils.MatVecMult(S, thetaVec)
		fmt.Printf("[NoPack-ILC] Loaded theta from %s\n", thetaCSVPath)
	} else {
		fmt.Println("[NoPack-ILC] Starting from zero feedforward.")
	}
	fmt.Printf("[NoPack-ILC] Feedforward ready: step0=[%.4f, %.4f]\n", vIlcVec[0], vIlcVec[1])
	...
	// ── Output directory ──────────────────────────────────────────────────
	outDir := filepath.Join(baseDir, "result_timing_nopacking")
	os.RemoveAll(outDir)
	...
	ilcTimeHistory := make([][]float64, 0, N_TRIALS)
	saveTrajAt := map[int]bool{1: true, N_TRIALS: true}

	for trial := 1; trial <= N_TRIALS; trial++ {
		fmt.Printf("\n=== NoPack-ILC Trial %d / %d ===\n", trial, N_TRIALS)

		xpLoop := make([]float64, n)
		z0Bar := utils.RoundVec(utils.ScalVecMult(1.0/(scaleR*scaleS), make([]float64, n)))
		zCtPack := RLWE.EncPack(z0Bar, tauCtrl, 1/scaleL, *encryptorRLWE, ringQ, params)
		...
		// ── Encrypted control loop (packed, same as simulation_packed.go) ──
		for k := 0; k < nSteps; k++ {
			ref := computeRef(k, Ts, trajTime)
			rk := []float64{ref[0]}
			...
			yMeasBar := utils.RoundVec(utils.ScalVecMult(1/scaleR, y))
			rMeasBar := utils.RoundVec(utils.ScalVecMult(1/scaleR, rk))
			rStateBar := utils.RoundVec(utils.ScalVecMult(1/(scaleR*scaleS), rk))

			yCt := RLWE.Enc(yMeasBar, 1/scaleL, *encryptorRLWE, ringQ, params)
			rkCt_meas := RLWE.Enc(rMeasBar, 1/scaleL, *encryptorRLWE, ringQ, params)
			yScalarStore[k] = cloneCtSlice(yCt)
			rScalarStore[k] = cloneCtSlice(rkCt_meas)
			rkCt_state := RLWE.Enc(rStateBar, 1/scaleL, *encryptorRLWE, ringQ, params)
			...
			u := RLWE.DecUnpack(uCtPack, m, tauCtrl, *decryptorRLWE, scaleR*scaleS*scaleS*scaleL, ringQ, params)
			for i := 0; i < m; i++ {
				u[i] += vIlcVec[m*k+i]
			}

			uReEnc := RLWE.Enc(
				utils.RoundVec(utils.ScalVecMult(1/scaleR, u)),
				1/scaleL, *encryptorRLWE, ringQ, params)
		...
		fmt.Printf("[NoPack-ILC] Trial %2d  Output RMS=%.6f\n", trial, rmsE)

		// Save V applied this trial (before update)
		...
		// ── NoPacking ILC update: Theta += K_r × E ────────────────────────
		fmt.Println("[NoPack-ILC] Building ctE and running encrypted update...")

		// Build ctE via ciphertext subtraction (same as packed)
		...
		// Time only RGSW×RLWE multiply
		tILC := time.Now()
		dThetaCt := RGSW.Mult(ctE, ctKr, evaluatorRGSW, ringQ, params)
		ilcElapsed := time.Since(tILC)
		ilcTimeHistory = append(ilcTimeHistory, []float64{float64(ilcElapsed.Milliseconds())})
		fmt.Printf("[NoPack-ILC] Mult done in %v\n", ilcElapsed.Round(time.Millisecond))

		// Theta accumulation (outside timing)
		for i := 0; i < nR; i++ {
		...
		// Decrypt Theta → reconstruct V = S @ Theta
		thetaVec = RLWE.Dec(ThetaEncCt, *decryptorRLWE, scaleS*scaleR*scaleL, ringQ, params)
		vIlcVec = utils.MatVecMult(S, thetaVec)
		fmt.Printf("[NoPack-ILC] Next feedforward: step0=[%.4f, %.4f]\n", vIlcVec[0], vIlcVec[1])

		// Persist plaintext theta for cross-run continuity
		if trial%100 == 0 || trial == N_TRIALS {
			utils.DataExport([][]float64{thetaVec}, thetaCSVPath)
			utils.DataExport(errHistory, filepath.Join(outDir, "error_rms.csv"))
			utils.DataExport(ilcTimeHistory, filepath.Join(outDir, "ilc_time_ms.csv"))
	...
	fmt.Printf("  ilc_time_ms.csv    : %d trials x [ILC update ms]\n", N_TRIALS)
	fmt.Println("Compare ilc_time_ms.csv with simulation_packed.go timing.")
}
