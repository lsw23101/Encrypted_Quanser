...
// simulation_svd.go — Encrypted ILC with SVD Reduced-Space Update
...
func main() {
	...
	ilcStateDir := filepath.Join(baseDir, "ilc_state_timing_packed")
	...
	if skData, err := os.ReadFile(skPath); err == nil {
		if err2 := sk.UnmarshalBinary(skData); err2 != nil {
			...
			os.Remove(filepath.Join(ilcStateDir, "theta_packed.bin"))
		} else {
			keyLoaded = true
	...
	thetaPath := filepath.Join(ilcStateDir, "theta_packed.bin")
	...
	// ── Output directory ──────────────────────────────────────────────────
	outDir := filepath.Join(baseDir, "result_timing_packed")
	os.RemoveAll(outDir)
	...
	for trial := 1; trial <= N_TRIALS; trial++ {
		...
		fmt.Println("[SVD-ILC] Building ctE and running encrypted update...")

		// Build ctE via ciphertext subtraction
	...
	fmt.Println("Run: python plot_svd_results.py")
}
