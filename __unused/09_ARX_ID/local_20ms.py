...
F = np.array([
    [1.227835, -1.862843, 2.181638, -1.590615],
    [1.109098, -1.682699, 1.067368, -0.436796],
    [1.804932, -2.738401, 1.737021, -2.338221],
    [1.992252, -3.022599, 1.917293, -2.580888]
], dtype=np.float64)
...
def control_loop():
    ...
    with QubeClass(hardware=hardware, pendulum=pendulum, frequency=frequency) as myQube:
        ...
        while timeStamp < simulationTime and not KILL_THREAD:
            ...
            u = float(H @ xhat)
            # u = float(H @ xhat)
            VMAX = 3.0 # high gain obsv에서는 이걸로
