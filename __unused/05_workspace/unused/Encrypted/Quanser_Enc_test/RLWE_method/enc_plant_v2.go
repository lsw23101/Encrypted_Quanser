// go_plant_bridge_enc_namedpipe.go
package main
...
// ====== 주소/파이프 이름 ======
const (
	pipeName         = `\\.\pipe\qube_yu` // Python ↔ Go (Named Pipe 서버)
	addrEncController = "127.0.0.1:9100"  // Go ↔ 암호 컨트롤러 (TCP 클라이언트)
...
const (
	...
	// quantization scales (기존 코드 값 유지)
	s      = 1.0 / 10.0
	L      = 1.0 / 10000.0
	r_quan = 1.0 / 1000.0
)
...
func main() {
	...
	// ================= Python 수신용 Named Pipe 서버 오픈 =================
	ln, err := winio.ListenPipe(pipeName, nil)
	if err != nil {
		log.Fatalf("[Go] ListenPipe error: %v", err)
	}
	defer ln.Close()
	fmt.Println("[Go] Controller server (Named Pipe) listening on", pipeName)
	...
	for {
		...
		// conn은 net.Conn 인터페이스이므로 TCP 옵션은 불가. 버퍼만 래핑해서 사용.
...
func handlePythonConn(
	conn net.Conn,
	encryptor *rlwe.Encryptor, // core/rlwe는 포인터 타입 반환
	...
	defer conn.Close()
	fmt.Println("[Go] New Python client (pipe):", conn.RemoteAddr())
