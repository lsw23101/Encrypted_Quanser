...
def control_loop():
    ...
    # 샘플링타임
    # frequency = 500# 500Hz일때 샘플링 타임 2ms로 제어
    ...
    with QubeClass(hardware=hardware, pendulum=pendulum, frequency=frequency) as myQube:
        ...
        while timeStamp < simulationTime and not KILL_THREAD:
            ...
            # 출력 4개 벡터
            y = np.array([theta, alpha, theta_dot, alpha_dot], dtype=np.float64)
            ...
            if alpha_degrees > 10:
                ...
                # voltage = 0
                VMAX = 10.0
                voltage = np.dot(K, y)          # 스칼라
                voltage = float(np.clip(voltage, -VMAX, VMAX))
            ...
            count += 1
            if count >= countMax:
                scopePendulum.sample(timeStamp, [y[1]])  # alpha (upright 기준)
                scopeBase.sample(timeStamp,     [y[0]])  # theta
