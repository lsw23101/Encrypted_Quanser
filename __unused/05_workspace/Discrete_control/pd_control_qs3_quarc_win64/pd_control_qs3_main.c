...
typedef RT_MODEL_pd_control_qs3_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * pd_control_qs3(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(pd_control_qs3_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(pd_control_qs3_DW.HILReadTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://pd_control_qs3:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: pd_control_qs3 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(pd_control_qs3_DW.HILReadTimebase_Task, (t_clock)
        pd_control_qs3_P.HILReadTimebase_Clock, 500.0, -1)
        ;
      if (result == 0) {
