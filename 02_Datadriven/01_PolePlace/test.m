...
% 1. 기준: place 함수를 이용한 OCF 게인 직접 산출
des_poles_K = [0.61, 0.62, 0.63, 0.64]; 
K_place = place(Ad_ident, Bd_ident, des_poles_K);

% 2. 뺄셈 방식: CCF 기반 Manual Gain 산출
% 목표 특성 다항식 계수 [1, delta1, delta2, delta3, delta4]
poly_des_K = poly(des_poles_K); 

% K_ccf = [목표 계수 - 현재 계수] (차수 순서 주의: d4, d3, d2, d1 순서로 대응)
% den은 식별된 모델의 계수 [1, d1, d2, d3, d4] 입니다.
K_ccf_manual = fliplr(poly_des_K(2:5) - den(2:5)); 

% 3. 좌표 변환: CCF 게인을 OCF 게인으로 변환 (x_ocf = T * x_ccf)
Co_ocf = ctrb(Ad_ident, Bd_ident);
Co_ccf = ctrb(A_std_ccf, B_std_ccf);
T = Co_ocf / Co_ccf;  % T = Co_ocf * inv(Co_ccf)

% K_ocf = K_ccf * inv(T)
K_ocf_transformed = K_ccf_manual / T; 

% 4. 최종 결과 비교 (H_ocf는 변환된 값을 사용)
fprintf('\n======================================================\n');
fprintf('      [6] 제어 게인 일치성 검증 (place vs Manual-T)\n');
fprintf('======================================================\n');
fprintf('A. place(OCF) 직접 구한 K:  [%s]\n', num2str(K_place, '%.4f '));
fprintf('B. 뺄셈 후 T 변환한 K:      [%s]\n', num2str(K_ocf_transformed, '%.4f '));
fprintf('두 게인 사이의 수치 오차:   %.2e\n', norm(K_place - K_ocf_transformed));

% 시뮬레이션용 행렬 할당 (변환된 게인이 똑같으므로 K_place를 써도 무방)
K = K_place; 
L_sub = [A1_est; A2_est];
F_ocf = Ad_ident - Bd_ident*K - L_sub*Cd_ident;
G_ocf = L_sub;
...
% CCF 기반 제어기 (검증 완료용으로만 유지)
K_ccf = K_ccf_manual; 
L_ccf = T \ L_sub; % inv(T) * L_sub
