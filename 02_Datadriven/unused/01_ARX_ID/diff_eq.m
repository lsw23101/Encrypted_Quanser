...
%% 5. 수치적 비교 출력 (벡터-행렬 수식 버전)
fprintf('\n======================================================================\n');
fprintf('           차분방정식 계수 비교 분석 (Reference vs Identified)\n');
fprintf('           모델 형태: y[k] = A1*y[k-1] + A2*y[k-2] + B1*u[k-1] + B2*u[k-2]\n');
fprintf('======================================================================\n');

% --- A1, A2 Matrix 비교 ---
fprintf('\n[1] 시스템 행렬 (A1, A2) 비교\n');
fprintf('    A1_Ref (정답)         A1_Ident (식별)        A2_Ref (정답)         A2_Ident (식별)\n');
for i = 1:2
    fprintf(' [%6.4f  %6.4f]    [%6.4f  %6.4f]    [%6.4f  %6.4f]    [%6.4f  %6.4f]\n', ...
        A1_ref(i,:), A1_est(i,:), A2_ref(i,:), A2_est(i,:));
end

% --- B1, B2 Vector 비교 ---
fprintf('\n[2] 입력 벡터 (B1, B2) 비교\n');
fprintf('    B1_Ref   B1_Ident      B2_Ref   B2_Ident\n');
for i = 1:2
    fprintf('   [%6.4f]   [%6.4f]      [%6.4f]   [%6.4f]\n', ...
        B1_ref(i), B1_est(i), B2_ref(i), B2_est(i));
end

% --- 행렬 형태의 수식 출력 ---
fprintf('\n[3] 식별된 모델의 벡터-행렬 차분방정식\n');
fprintf('----------------------------------------------------------------------\n');
fprintf(' [ theta(k) ]   [ %7.4f  %7.4f ] [ theta(k-1) ]   [ %7.4f  %7.4f ] [ theta(k-2) ]\n', ...
    A1_est(1,1), A1_est(1,2), A2_est(1,1), A2_est(1,2));
fprintf(' [ alpha(k) ] = [ %7.4f  %7.4f ] [ alpha(k-1) ] + [ %7.4f  %7.4f ] [ alpha(k-2) ]\n', ...
    A1_est(2,1), A1_est(2,2), A2_est(2,1), A2_est(2,2));
fprintf('\n');
fprintf('                 [ %7.4f ]               [ %7.4f ]\n', B1_est(1), B2_est(1));
fprintf('               + [ %7.4f ] * u(k-1)    + [ %7.4f ] * u(k-2)\n', B1_est(2), B2_est(2));
fprintf('----------------------------------------------------------------------\n');
fprintf('======================================================================\n');
